...
var _ = Describe("backup integration create statement tests", func() {
	...
	Describe("PrintTypeStatements", func() {
		...
		It("creates composite types", func() {
			...
			var excludeOIDs []string
			if connection.Version.Before("5") {
				excludeOIDs = []string{"0"} // No autogenerated arrays for composite types in GPDB4
			} else {
				oid := testutils.OidFromObjectName(connection, "public", "_composite_type", backup.TYPE_TYPE)
		...
		It("creates base types", func() {
			...
			oid := testutils.OidFromObjectName(connection, "public", "_base_type", backup.TYPE_TYPE)
			excludeOIDs := []string{fmt.Sprintf("%d", oid)}
			resultTypes := backup.GetNonEnumTypes(connection, excludeOIDs)
