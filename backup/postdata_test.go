...
var _ = Describe("backup/postdata tests", func() {
	...
	Context("PrintCreateIndexStatements", func() {
		It("can print a basic index", func() {
			indexes := []backup.QuerySimpleDefinition{{Oid: 1, Name: "testindex", OwningSchema: "public", OwningTable: "testtable", TablespaceName: "", Def: "CREATE INDEX testindex ON public.testtable USING btree(i)"}}
		...
		It("can print an index with a tablespace", func() {
			indexes := []backup.QuerySimpleDefinition{{Oid: 1, Name: "testindex", OwningSchema: "public", OwningTable: "testtable", TablespaceName: "test_tablespace", Def: "CREATE INDEX testindex ON public.testtable USING btree(i)"}}
			emptyMetadataMap := backup.MetadataMap{}
			backup.PrintCreateIndexStatements(backupfile, toc, indexes, emptyMetadataMap)
			testutils.AssertBufferContents(toc.PostdataEntries, buffer, `CREATE INDEX testindex ON public.testtable USING btree(i);
...
COMMENT ON INDEX testindex IS 'This is an index comment.';`)
	...
	Context("PrintCreateRuleStatements", func() {
		It("can print a basic rule", func() {
			rules := []backup.QuerySimpleDefinition{{Oid: 1, Name: "testrule", OwningSchema: "public", OwningTable: "testtable", TablespaceName: "", Def: "CREATE RULE update_notify AS ON UPDATE TO testtable DO NOTIFY testtable;"}}
			emptyMetadataMap := backup.MetadataMap{}
...
COMMENT ON RULE testrule ON public.testtable IS 'This is a rule comment.';`)
	...
	Context("PrintCreateTriggerStatements", func() {
		It("can print a basic trigger", func() {
			triggers := []backup.QuerySimpleDefinition{{Oid: 1, Name: "testtrigger", OwningSchema: "public", OwningTable: "testtable", TablespaceName: "", Def: "CREATE TRIGGER sync_testtable AFTER INSERT OR DELETE OR UPDATE ON testtable FOR EACH STATEMENT EXECUTE PROCEDURE flatfile_update_trigger()"}}
			emptyMetadataMap := backup.MetadataMap{}
